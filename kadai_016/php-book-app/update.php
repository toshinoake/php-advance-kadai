<?php
  ...
  if (isset($_POST['submit'])) {
    try {
        ...
        //動的に変わる値をプレースホルダに置き換えたUPDATET文をあらかじめ用意する
        $sql_update = '
            UPDATE books
  ...
  if (isset($_GET['id'])) {
      try {
          ...
          //idカラムの値をプレースホルダに（:id)に置き換えたSQL文をあらかじめ用意する
          ...
          //genresテーブルからgenre_codeカラムのデータを取得するためのSQL文を$sql_select_genre_codesに代入する
          ...
          //SQL文の実行結果を配列で取得する
          //補足；PDO::FETCH_COLUMNは１つのカラムの値を1次元配列（多次元ではない普通の配列）で取得する設定である
      ...
      <article class="registration">
        ...
        <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
          <div>
            ...
            <input type="numver" name="price" value="<?= $book['price'] ?>" min="0" max="100000000" required>
